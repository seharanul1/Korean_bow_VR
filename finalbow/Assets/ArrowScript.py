...
class ArrowScript(Actor.Actor):
    ...
    def OnCreate(self,uid):
        ...
        if self.time == 0.0 :
            self.init_coord_cov = Math3d.Vector3(self.coord_cov.x, self.coord_cov.y, self.coord_cov.z)
            self.w_angle = math.degrees(self.rot_cov[1])
            self.h_angle = math.degrees(self.rot_cov[2])
            print(self.w_angle, self.h_angle)
            
            # 화살의 속도
            self.initV = [math.cos(math.pi / 180 * self.h_angle) * math.cos(math.pi / 180 * self.w_angle), math.sin(math.pi / 180 * self.h_angle), math.cos(math.pi / 180 * self.h_angle) * math.sin(math.pi / 180 * self.w_angle)]
    ...
    def Update(self):
        ...
        if(self.shoot):
            ...
            self.rot_cov = (0, math.acos( self.curV[0] / self.tempXZ) * self.w, math.acos(self.tempXZ /self.curV_mag) * self.h )
